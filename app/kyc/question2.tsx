...
import { QUESTION_2 } from "@/static/data/kycQuestion";

interface question2Props {}

const question2: React.FC<question2Props> = () => {
  const KYCctx = useContext(KYCContext);
  const [value, setValue] = React.useState<string>(KYCctx.question2);
  const handleSelect = (value: string) => {
    KYCctx.setQuestion2(value);
    setValue(value);
  };
  return (
    <>
      ...
      <SafeAreaView className="flex-1 bg-white">
        ...
        <View className="w-full flex-1 pt-8 px-[5%]">
          <Text className="text-xl">{QUESTION_2.question}</Text>
          <View className="w-full mt-6">
            {QUESTION_2.options.map((item, index) => {
              return (
                <RadioButtonItem
                  key={index}
                  title={item}
          ...
          >
            <View className="overflow-hidden rounded-xl w-full">
              <Pressable
                ...
                onPress={() => router.push("/kyc/question3")}
